                    ...
                    <?php if (isset($listaAnios) && is_array($listaAnios)): ?>
                        <?php foreach ($listaAnios as $anioInfo): ?>
                            <?php
                                ...
                                // Use the correct filter variable from controller
                    ...
                    <?php else: ?>
                        <?php foreach ($listaVacaciones as $vacacion): ?>
                            <tr>
                                ...
                                <td><?php echo htmlspecialchars($vacacion['area'] ?? 'N/A'); ?></td> <td><?php echo htmlspecialchars($vacacion['fecha_inicio'] ?? 'N/A'); ?></td>
                                ...
                                <td class="text-center">
                                    <?php /* ... Badge logic remains the same ... */
                                        $estado = htmlspecialchars($vacacion['estado'] ?? 'N/A');
                                        $claseBadge = 'text-bg-secondary';
                                ...
                                <td class="text-center">
                                    <?php $vacacion_id = $vacacion['id'] ?? null; if($vacacion_id): ?>
                                    <a href="index.php?controller=vacacion&action=edit&id=<?php echo $vacacion_id; ?>" class="btn btn-warning btn-sm" title="Editar"><i class="bi bi-pencil-fill"></i></a>
                                    <a href="index.php?controller=vacacion&action=delete&id=<?php echo $vacacion_id; ?>" class="btn btn-danger btn-sm" title="Cancelar/Eliminar" onclick="return confirm('¿Estás seguro de que deseas eliminar este registro?');"><i class="bi bi-trash-fill"></i></a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
